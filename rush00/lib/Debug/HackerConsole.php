<?php
/**
 * Debug_HackerConsole: write debug messages into hidden console.
...
class Debug_HackerConsole
{
     ...
     * HTML page.
     */
    function Debug_HackerConsole($autoAttach=false)
    {
        if ($autoAttach) ob_start(array(&$this, '_obHandler'));
        $GLOBALS['Debug_HackerConsole_LAST'] =& $this;
    }
    ...
    function out($v, $group="message", $color=null, $tip=null)
    {
        ...
        if (empty($this) || strtolower(get_class($this)) != 'debug_hackerconsole') {
            $obj =& $GLOBALS['Debug_HackerConsole_LAST'];
        } else {
            $obj =& $this;
        }
        if (!$obj) return;
        ...
        else $text = Debug_HackerConsole::print_r($v, true);
    ...
    function toPre($text, $tabSize=null)
    {
        ...
        // Expand tabulators.
        if ($tabSize === null) {
            if (isset($GLOBALS['Debug_HackerConsole_LAST'])) 
                $tabSize = $GLOBALS['Debug_HackerConsole_LAST']->TAB_SIZE;
            else 
                $tabSize = 4;
        }
        $text = Debug_HackerConsole::expandTabs($text, $tabSize);
        ...
        if ($level < 10) {
            ...
            if (is_array($obj) || is_object($obj)) { 
                ...
                for (reset($obj); list($k, $v) = each($obj); ) {
                    ...
                    $buf .= "\n{$leftSp}[$k] => ".Debug_HackerConsole::print_r($v, $no_print, $level+1);
                }
            }
        } else {
            $buf = "*RECURSION*";
    ...
    function expandTabs($text, $tabSize=4)
    {
        ...
        while (1) {
            $old = $text;
            $text = preg_replace_callback('/^([^\t\r\n]*)\t(\t*)/m', array('Debug_HackerConsole', 'expandTabs_callback'), $text);
 ...
 */
$GLOBALS['Debug_HackerConsole_LAST'] = null;
?>
